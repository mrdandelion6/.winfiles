...
if (-Not (Test-Path $device_register)) {
    Write-Host "device register file: $device_register not found. create it or configure profile.ps1" -ForegroundColor Red
...
$default_envdir = 'C:\envs' # by default, we use this directory for virtual environments
switch ($device_name) {
    ...
    default { 
        Write-Host "device name not recognized, using default env directory: $envdir" -ForegroundColor Yellow
        $envdir = $default_envdir
    }
}

if (-Not (Test-Path $envdir)) {
        Write-Host "env directory not found: $envdir. create it or configure profile.ps1" -ForegroundColor Red
}
